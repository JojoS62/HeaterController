...
#if LV_USE_FS_FATFS
    #define DIR FATFS_DIR                   /* MBed typedefs `DIR` as `FF_DIR` in its version of ff.h. Use `FF_DIR` in LVGL too */

    #define LV_FS_FATFS_LETTER 'A'          /**< Set an upper cased letter on which the drive will accessible (e.g. 'A') */
    #define LV_FS_FATFS_PATH    "//sda/"    /**< Set the working directory. File/directory paths will be appended to it. */
